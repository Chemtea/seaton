...
function seatingSVG(input, teacher=false) {
  ...
  const block=(x,y,w,h,label,kind='seat')=>{
    const units=[...label].reduce((sum,char)=>sum+(/[\u2e80-\uffff]/u.test(char)?1:.7),0);
    const p=point(x,y),size=kind==='seat'?Math.max(11,Math.min(28,(w-12)/Math.max(3,units))):20;
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 ${height}" role="img" aria-label="${teacher?'교탁용':'학생용'} 자리표"><style>text{font-family:'Malgun Gothic','맑은 고딕','Noto Sans CJK KR',sans-serif;fill:#101b2d}rect{fill:#fff;stroke:#718096;stroke-width:1.4}.board rect{fill:#e8eef4;stroke:#8896a7}.podium rect{fill:#edf0f3;stroke:#8896a7}.group-label{font-size:19px;font-weight:700;fill:#283b55}.empty rect{stroke:#bbc3cd;stroke-dasharray:4 4}.seat text{font-weight:700}</style>${block(500,31,370,40,'칠판','board')}${block(500,101,165,48,'교탁','podium')}${groups}${seats}</svg>`;
}
